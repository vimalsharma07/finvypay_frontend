...
/**
 * POST https://{API}/api/production/status/{transactionId}
 ...
 */
export async function postProductionPaymentStatus(transactionId: string): Promise<any> {
  const base = (process.env.NEXT_PUBLIC_API_URL || '').replace(/\/+$/, '');
  if (!base) {
    throw new Error('NEXT_PUBLIC_API_URL is not configured');
  }
  const url = `${base}/api/production/status/${encodeURIComponent(transactionId.trim())}`;
  return http.post(url, undefined, { auth: false });
}
