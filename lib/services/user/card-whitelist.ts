/**
 * User - Card Whitelist API Service
 * 
 * Centralized API calls for user card whitelist management
 */

import { http, ApiError } from '../../api';
import { userCardWhitelistRoutes } from '../../routes/user/card-whitelist-routes';
...
export async function getCardWhitelistById(
  ...
  try {
    const data = await http.get(userCardWhitelistRoutes.getById(id)) as CardWhitelist;
    return {
      status: 200,
      data,
    };
...
export async function createCardWhitelist(
  ...
  try {
    const data = await http.post(userCardWhitelistRoutes.create, payload) as CardWhitelist;
    return {
      status: 200,
      data,
    };
...
export async function updateCardWhitelist(
  ...
  try {
    const data = await http.put(userCardWhitelistRoutes.update(id), payload) as CardWhitelist;
    return {
      status: 200,
      data,
    };
...
export async function deleteCardWhitelist(id: string | number): Promise<ApiResponse<void>> {
  try {
    const response = await http.delete(userCardWhitelistRoutes.delete(id));
