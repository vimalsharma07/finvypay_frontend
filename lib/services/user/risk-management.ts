/**
 * User - Risk Management API Service
 * 
 * Centralized API calls for user risk management operations
 */

import { http, ApiError } from '../../api';
import { userRiskManagementRoutes } from '../../routes/user/risk-management-routes';
...
export async function getRiskManagement(
  ...
  try {
    const data = await http.get(userRiskManagementRoutes.list, {
...
export async function getRiskManagementById(
  ...
  try {
    const data = await http.get(userRiskManagementRoutes.getById(id)) as RiskManagement;
    return {
      status: 200,
      data,
    };
...
export async function createRiskManagement(
  ...
  try {
    const data = await http.post(userRiskManagementRoutes.create, payload) as RiskManagement;
    return {
      status: 200,
      data,
    };
...
export async function updateRiskManagement(
  ...
  try {
    const data = await http.put(userRiskManagementRoutes.update(id), payload) as RiskManagement;
    return {
      status: 200,
      data,
    };
...
export async function deleteRiskManagement(id: string | number): Promise<ApiResponse<void>> {
  try {
    const response = await http.delete(userRiskManagementRoutes.delete(id));
...
export async function getRiskTypes(): Promise<ApiResponse<RiskTypesResponse>> {
  try {
    const data = await http.get(userRiskManagementRoutes.types) as RiskTypesResponse;
    return {
      status: 200,
      data,
    };
