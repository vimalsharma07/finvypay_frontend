/**
 * Merchant Notifications API Service
 * 
 * Centralized API calls for merchant notification operations
 */
...
/**
 * Get unread notification count for merchant
 * 
 * @returns Promise with unread count response
 */
export async function getMerchantUnreadCount(): Promise<ApiResponse<UnreadCountResponse>> {
  try {
    const data = await http.get(routes.merchant.notifications.unreadCount) as UnreadCountResponse;
    return {
      status: 200,
      data,
    };
...
export async function markNotificationAsRead(
  ...
  try {
    const data = await http.put(routes.merchant.notifications.markAsRead(id));
...
export async function markAllNotificationsAsRead(): Promise<ApiResponse<{ success: boolean; message?: string }>> {
  try {
    const data = await http.put(routes.merchant.notifications.markAllAsRead);
...
export async function deleteNotification(
  ...
  try {
    const data = await http.delete(routes.merchant.notifications.delete(id));
    return {
      status: 200,
