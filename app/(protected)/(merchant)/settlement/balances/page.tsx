...
import {
  getUserSettlementBalanceHistory,
  UserSettlementBalance,
...
export default function UserSettlementBalancesPage() {
  const [data, setData] = useState<UserSettlementBalance[]>([]);
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const sortBy = sorting[0]?.id || 'transactionDate';
      const sortOrder = sorting[0]?.desc ? 'DESC' : 'ASC';

      const response = await getUserSettlementBalanceHistory({
        page,
        limit,
        sortBy,
        sortOrder,
      ...
      handleApiResponse(response, {
        onSuccess: (res) => {
          if (res && res.success && res.data) {
            ...
            toast.error('Invalid response structure while loading balance history');
          }
        },
        onError: (message) => {
          toast.error(message || 'Failed to load balance history');
        },
        silent: true,
      });
    } catch (error) {
      toast.error('An unexpected error occurred while fetching balance history');
  ...
  const columns = useMemo<ColumnDef<UserSettlementBalance>[]>(
    () => [
      {
      ...
      <Container>
        <Toolbar>
          <ToolbarHeading
            ...
            description="View your settlement balance history with opening, net, payout, and closing balances"
            icon={Wallet}
          />
          <ToolbarActions>
