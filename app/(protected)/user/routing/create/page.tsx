...
export default function CreateRoutingPage() {
  ...
  const onSubmit = async (data: CreateRoutingFormData): Promise<void> => {
    try {
      setIsLoading(true);
      console.log('Submitting routing data:', data); // Debug log
      ...
      const response = await createUserMerchantRouting(processedData);

      handleApiResponse(response, {
        onSuccess: () => {
          toast.success('Routing rule created successfully');
          router.push(`/user/routing?profileId=${profileId}`);
        },
        onError: (errorMessage) => {
