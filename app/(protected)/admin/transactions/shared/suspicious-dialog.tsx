...
type SuspiciousFormData = z.infer<typeof suspiciousSchema>;
...
export function SuspiciousDialog({
  open,
  onOpenChange,
  transaction,
  onSubmit,
  isSubmitting = false,
}: SuspiciousDialogProps) {
  const form = useForm<SuspiciousFormData>({
  ...
  const handleSubmit = async (data: SuspiciousFormData) => {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Mark transaction {transaction.transactionId} as suspicious
          </DialogDescription>
        </DialogHeader>
          ...
          <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-4">
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Textarea
                        placeholder="Enter reason for marking as suspicious (e.g., Fraudulent activity detected)"
            ...
            <DialogFooter>
              ...
              <Button type="submit" disabled={isSubmitting}>
                {isSubmitting ? 'Processing...' : 'Mark as Suspicious'}
              </Button>
            </DialogFooter>
          </form>
        </Form>
