...
export default function ConnectorsPage() {
  ...
  const [connectors, setConnectors] = useState<MerchantGateway[]>([]);
  const [meta, setMeta] = useState<
    MerchantGatewayListResponse['data']['meta'] | null
  >(null);
  ...
  const fetchConnectors = async (
    ...
    try {
      ...
      handleApiResponse<MerchantGatewayListResponse>(response, {
        onSuccess: (data) => {
          if (data.success) {
  ...
  const headers: TableHeader<MerchantGateway>[] = [
    { key: 'name', label: 'Name', sortable: true },
    { key: 'terminalId', label: 'Terminal ID', sortable: true },
    { key: 'gateway', label: 'Gateway', sortable: false },
    { key: 'paymentChannel', label: 'Payment Channel', sortable: false },
  ...
  const renderCell = (
    item: MerchantGateway,
    key: keyof MerchantGateway | string,
  ) => {
    switch (key) {
      case 'name':
      ...
      default:
        const value = item[key as keyof MerchantGateway];
        return (
  ...
  // Define actions
  const actions: TableAction<MerchantGateway>[] = [
    {
      label: 'View',
      route: (row: MerchantGateway) =>
        `/admin/merchant-gateway/${row.id}`,
    },
    {
      label: 'Edit',
      route: (row: MerchantGateway) =>
        `/admin/merchant-gateway/${row.id}/edit`,
    },
    {
      label: 'Delete',
      onClick: async (row: MerchantGateway) => {
    ...
    <Fragment>
          ...
          <ToolbarActions>
            <Button
              ...
              onClick={() => {
                ...
                const returnUrl = encodeURIComponent(`/admin/user-management/merchant/${userId}/routing_cascading/payment-channels`);
                window.location.href = `/admin/user-management/merchant/${userId}/routing_cascading/payment-channels/create?returnUrl=${returnUrl}`;
              }}
            >
      ...
      <Container>
        <TableComp
          ...
          getRowId={(row: MerchantGateway) => String(row.id)}
          pagination={{
            pageSize: limit,
            pageIndex: page - 1,
